...
const ProductDetailsCarousel = ({ product }: Props) => {
  const theme = useTheme();
  const [slides, setSlides] = useState<any[]>([]);
  // product.productGallery.map(img => ({
  //   src: img.image,
  // }));
  useEffect(() => {
    (async () => {
      const newSlides = product.productGallery.map((img: { image: any }) => ({
        src: img.image,
      }));
      ...
      await setSlides(prev => [...prev, ...newSlides]);
    })();
  }, [product]);
